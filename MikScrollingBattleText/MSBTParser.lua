...
-- ****************************************************************************
-- Refreshes cached player and pet GUID/name.
-- In Retail, UnitGUID("player") can be nil during early load.
-- ****************************************************************************
...
local function ParseLogMessage(timestamp, event, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, recipientGUID, recipientName, recipientFlags, recipientRaidFlags, ...)
	...
	-- Retail safety: fall back to GUID comparisons if the unit map isn't ready yet.
	...
	if not recipientUnit then
		if playerGUID and recipientGUID == playerGUID then
			recipientUnit = "player"
		elseif petGUID and recipientGUID == petGUID then
			recipientUnit = "pet"
		end
	end
...
local function OnEvent(this, event, arg1, arg2, ...)
	...
	-- Player entering world: refresh GUID caches (Retail load order).
...
local function Enable()
	-- Refresh cached GUIDs now that we're logged in.
	RefreshPlayerInfo()
