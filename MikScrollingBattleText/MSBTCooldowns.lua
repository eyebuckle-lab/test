...
-- ****************************************************************************
-- Normalizes cooldown return values across Retail API variants.
-- Some APIs return tuples (start, duration, enabled) while modern Retail may return a table.
-- Also guards against nil/"secret" tainted values by coercing via tonumber().
-- ****************************************************************************
local function NormalizeCooldownReturn(cooldownFunc, cooldownID)
    -- Retail 12.x safety:
    -- Cooldown APIs may return tuples, tables, nils, and in some cases "secret" values that cannot be compared.
    -- We avoid touching the 'enabled' return entirely and instead infer enabled from duration > 0.
    local a, b = cooldownFunc(cooldownID)

    if a == nil then
        return nil, nil, false
    end

    -- Table return (common for C_ APIs).
    if type(a) == "table" then
        local cd = a
        local startTime = cd.startTime or cd.start or cd[1]
        local duration  = cd.duration  or cd[2]

        startTime = (startTime ~= nil) and tonumber(tostring(startTime)) or nil
        duration  = (duration  ~= nil) and tonumber(tostring(duration))  or nil

        if startTime == nil and duration == nil then
            return nil, nil, false
        end

        local enabled = (duration ~= nil and duration > 0) or false
        return startTime or 0, duration or 0, enabled
    end

    -- Tuple return (legacy globals).
    local startTime = (a ~= nil) and tonumber(tostring(a)) or nil
    local duration  = (b ~= nil) and tonumber(tostring(b)) or nil
...
local function GetCooldownTexture(cooldownType, cooldownID)
	if cooldownType == "item" then
		...
		if iconID then
			...
			return MSBT_GetSpellInfo(cooldownID)
		end
	end
end
...
local function OnSpellCast(unitID, spellID)
	...
	local spellName = MSBT_GetSpellInfo(spellID) or UNKNOWN
...
local function OnUpdateCooldown(cooldownType, cooldownFunc)
	...
	for cooldownID in pairs(delayedCooldowns[cooldownType]) do
		...
		local _, duration, enabled = NormalizeCooldownReturn(cooldownFunc, cooldownID)
		if enabled then
			...
			local cooldownName = MSBT_GetSpellInfo(cooldownID)
...
local function OnUpdate(frame, elapsed)
	...
	if lastUpdate >= updateDelay then
		...
		for cooldownType, cooldowns in pairs(activeCooldowns) do
			local cooldownFunc = (cooldownType == "item") and C_Container.GetItemCooldown or MSBT_GetSpellCooldown
			local infoFunc = (cooldownType == "item") and GetItemInfo or MSBT_GetSpellInfo
			for cooldownID, remainingDuration in pairs(cooldowns) do
				-- Ensure the cooldown is still valid.
				local startTime, duration, enabled = NormalizeCooldownReturn(cooldownFunc, cooldownID)
...
function eventFrame:SPELL_UPDATE_COOLDOWN()
	OnUpdateCooldown("player", MSBT_GetSpellCooldown)
end
...
function eventFrame:PET_BAR_UPDATE_COOLDOWN()
	OnUpdateCooldown("pet", MSBT_GetSpellCooldown)
end
